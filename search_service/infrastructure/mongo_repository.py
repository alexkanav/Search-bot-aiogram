...
class ItemRepository:
    ...
    async def get_item(self, user_id: int, card_id: str) -> list[ItemCard]:
        doc = await self._collection.find_one({"user_id": user_id, "card_id": card_id})
        return [ItemCard.model_validate(doc)] if doc else []
    ...
    async def insert_items(self, user_id: int, items: list[ItemCard]) -> None:
        ...
        expires_at = datetime.now(UTC) + timedelta(days=ITEM_TTL_DAYS)
...
async def create_mongodb_repository(settings) -> tuple[ItemRepository, AsyncIOMotorClient]:
